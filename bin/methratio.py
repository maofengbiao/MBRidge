...
#        print dd, dep0
	if options.CT_SNP > 0: 
        ...
        m = methcr[i]
#	print cr, i+1, strand, seq, d, m, dd, m1, d1
#        try: ratio = min(m, d) / d
#        except: ZeroDivisionError: continue
	if d == 0 and strand == '+' and dep0 == 0:
#			fout.write('%s\t%d\t%c\t%s\t%s\t%.2f\t%d\t%d\t%d\t%d\t%s\t%s\n' % (cr, i+1, strand, seq, 'NA', d,m, dd, m1, d1, 'NA', 'NA'))
			fout.write('%s\t%d\t%c\t%s\t%s\t%.1f\t%d\t%d\t%d\t%d\t%s\t%s\n' % (cr, i+1, strand, seq, 'NA', d, m, dd, m1, d1, 'NA', 'NA'))
#	else: ratio = min(m, d) / d
	try: ratio = min(m, d) / d
	except ZeroDivisionError: continue
        #print ratio
        nc += 1
